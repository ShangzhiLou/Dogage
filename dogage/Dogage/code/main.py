...
def create_model(model_name='mobilenet_v2', num_classes=3, pretrained=True, dropout_rate = 0.2):
    """创建模型"""
...
def train_model(train_df, val_df, model_name='mobilenet_v2', image_size=224, batch_size=32, num_epochs=20, model_path='best_dog_model.pth', use_gpu=True, use_face_detection=True, log_dir = 'log'):
    """训练模型"""
    ...
    torch.save(model.state_dict(), os.path.join(log_dir,model_path))
...
def eval_model(model, test_df, image_size=224, batch_size=32, use_gpu=True, use_face_detection=True, log_dir = 'log'):
    """测试模型"""

    # Create directory for saving testing results.
    log_dir = os.path.join(log_dir,model_name)
    os.makedirs(log_dir, exist_ok=True)
...
if __name__ == '__main__':
    ...
    for model_name in model_names:
        ...
        trained_model = train_model(train_df, val_df, model_name=model_name, image_size=image_size, batch_size=batch_size, num_epochs=num_epochs, model_path=model_path, use_face_detection=True, log_dir=log_dir)
        eval_model(trained_model, test_df, image_size=image_size, batch_size=batch_size, use_face_detection=True, log_dir = log_dir)